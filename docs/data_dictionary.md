...
| `output_message_length` | int64 | Same, for output messages. On synthetic rows this is inherited unchanged from the real predecessor — it does *not* get zeroed alongside `output_tokens`. Verified on a real example (`call_id` `8fc8f765411ba47c`): synthetic row has `output_message_length=1091` but `output_tokens=0`. Known inconsistency, not a bug: only the fields the injection design explicitly listed (`output_tokens`, `status_code`, `error_type`, timestamps) get overridden; unlisted fields are inherited as-is. |
